        ...
        <div className={`flex-shrink-0 w-80 rounded-[2rem] flex flex-col max-h-full border transition-all duration-500 ${isWipExceeded ? 'bg-red-50 border-red-200' :
            ...
            <div className="p-6 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <h3 className={`font-semibold text-[11px] uppercase tracking-[0.2em] ${isWipExceeded ? 'text-red-700' : 'text-[#1c1917]'}`}>{title}</h3>
                    <div className="flex items-center gap-2">
                        <span className={`text-[9px] font-medium px-2.5 py-0.5 rounded-full border tracking-widest ${isWipExceeded ? 'bg-red-100 border-red-300 text-red-700 shadow-sm shadow-red-500/10' :
                        ...
                        {isWipExceeded && (
                            <span className="text-[8px] font-medium text-red-500 uppercase tracking-tighter animate-pulse italic">
                                SATURATION REACHED
                            </span>
                        )}
                    </div>
            ...
            >
                ...
                {tasks.length === 0 && (
                    <div className="py-16 flex flex-col items-center justify-center border-2 border-dashed border-[#e5dec9]/60 rounded-[1.5rem] bg-white/30">
                        <p className="text-[10px] font-medium text-[#1c1917]/20 uppercase tracking-[0.2em] italic">Stream Inactive</p>
                    </div>
                )}
            </div>
