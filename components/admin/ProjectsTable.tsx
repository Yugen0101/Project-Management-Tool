                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-[#f7f3ed]/50 border-b border-[#e5dec9]">
                            <th className="px-8 py-5 text-[10px] font-medium uppercase tracking-[0.2em] text-[#1c1917]/40 w-16">ID</th>
                            <th className="px-8 py-5 text-[10px] font-medium uppercase tracking-[0.2em] text-[#1c1917]/40">Project Name</th>
                            <th className="px-8 py-5 text-[10px] font-medium uppercase tracking-[0.2em] text-[#1c1917]/40">Owner</th>
                            <th className="px-8 py-5 text-[10px] font-medium uppercase tracking-[0.2em] text-[#1c1917]/40">Status</th>
                            <th className="px-8 py-5 text-[10px] font-medium uppercase tracking-[0.2em] text-[#1c1917]/40 text-center">Tasks</th>
                            <th className="px-8 py-5 text-[10px] font-medium uppercase tracking-[0.2em] text-[#1c1917]/40 text-center">Phases</th>
                            <th className="px-8 py-5 text-[10px] font-medium uppercase tracking-[0.2em] text-[#1c1917]/40">Timeline</th>
                            <th className="px-8 py-5 text-[10px] font-medium uppercase tracking-[0.2em] text-[#1c1917]/40 text-right">Actions</th>
                        </tr>
                    </thead>
                        ...
                        {projects.length > 0 ? (
                            projects.map((project, idx) => (
                                <tr key={project.id} className={`group hover:bg-[#f7f3ed]/20 transition-all ${isActionPending === project.id ? 'opacity-50 pointer-events-none' : ''}`}>
                                    <td className="px-8 py-6 text-[11px] font-normal text-[#1c1917]/20">{(idx + 1).toString().padStart(2, '0')}</td>
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col gap-1">
                                            <Link
                                                href={`${basePath}/projects/${project.id}`}
                                                className="text-sm font-medium text-[#1c1917] hover:text-accent-500 transition-colors uppercase tracking-tight"
                                            >
                                                {project.name}
                                            </Link>
                                            <span className="text-[10px] text-[#1c1917]/30 font-medium italic truncate max-w-[200px]">
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-[11px] font-medium text-[#1c1917]/60 uppercase tracking-tight">System Admin</span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <span className={`badge inline-flex !px-3 !py-1 text-[9px] font-medium uppercase tracking-widest ${project.status === 'active' ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' :
                                            project.status === 'completed' ? 'bg-blue-500/10 text-blue-600 border-blue-500/20' :
                                                'bg-[#1c1917]/5 text-[#1c1917]/30 border-[#1c1917]/10'
                                            }`}>
                                            {project.status}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-2 text-[10px] font-medium text-[#1c1917]/40 uppercase tracking-widest">
                                    ...
                                    <td className="px-8 py-6 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <Link
                                                href={`${basePath}/projects/${project.id}`}
                                                title="View Project"
                                                className="p-2 text-[#1c1917]/20 hover:text-accent-500 transition-all bg-[#f7f3ed]/50 hover:bg-white border border-transparent hover:border-[#e5dec9] rounded-lg"
                                            >
                                            ...
                                            {isAdmin && project.status !== 'archived' && (
                                                <button
                                                    ...
                                                    className="p-2 text-[#1c1917]/20 hover:text-amber-600 transition-all bg-[#f7f3ed]/50 hover:bg-white border border-transparent hover:border-[#e5dec9] rounded-lg"
                                                >
                                            ...
                                            {isAdmin && (
                                                <button
                                                    ...
                                                    className="p-2 text-[#1c1917]/20 hover:text-red-500 transition-all bg-[#f7f3ed]/50 hover:bg-white border border-transparent hover:border-[#e5dec9] rounded-lg"
                                                >
