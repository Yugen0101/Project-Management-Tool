    ...
    return (
        <div className="relative">
            <button
                ...
                className="w-full py-4 bg-white text-primary-600 font-black rounded-2xl hover:bg-slate-50 transition-colors shadow-lg shadow-black/10 flex items-center justify-center gap-2"
            >
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl border border-[#e5dec9] shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="max-h-64 overflow-y-auto p-2 space-y-1">
                        <button
                            ...
                            className={`w-full text-left px-4 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-between ${!currentAssigneeId ? 'bg-primary-50 text-primary-600' : 'text-slate-400 hover:bg-slate-50'}`}
                        >
                            Unassigned
                        ...
                        {members.map((member) => (
                            <button
                                ...
                                className={`w-full text-left px-4 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-between ${currentAssigneeId === member.user.id ? 'bg-primary-50 text-primary-600' : 'text-slate-600 hover:bg-slate-50'}`}
                            >
